...
for k, file in enumerate(files):
    ...
    for line in f:
        ...
        cou += 1
    if not os.path.exists("data/"+hp.dataset+"/train/"+str(k)):
        os.makedirs("data/"+hp.dataset+"/train/"+str(k))
    cnt = 0
    for start_t in range(hp.timestep):
        filename = str(cnt)
        cnt += 1
        f = open("data/"+hp.dataset+"/train/"+str(k) + '/graph_' + filename + '.inp', 'w')
        for it in edge_sort[start_t]:
            f.write(it[0] + ' ' + it[1] + '\n')
        f.close()

files = glob.glob("raw-data/"+hp.dataset+"/*.test")
print("共有 %d 个图"%len(files))
for k, file in enumerate(files):
    print("处理第 %d 个图" % (k+1))
    f = open(file, 'r')
    ...
    rep_num = edge_cnt // 2
    cou = 0
    edge_sort = [[] for i in range(2)]
    G = [set() for i in range(2)]
    f = open(file, 'r')
    for line in f:
        temp = line[:-1].split(' ')
        # print(fn)
        ...
        edge_sort[time_stamp].append([biao[temp[0]], biao[temp[1]]])
        cou += 1
    if not os.path.exists("data/"+hp.dataset+"/test"):
        os.makedirs("data/"+hp.dataset+"/test")
    cnt = 0
    for start_t in range(2):
        filename = str(cnt)
        cnt += 1
        f = open("data/"+hp.dataset+"/test/graph_" + filename + '.inp', 'w')
        for it in edge_sort[start_t]:
            f.write(it[0] + ' ' + it[1] + '\n')
        f.close()